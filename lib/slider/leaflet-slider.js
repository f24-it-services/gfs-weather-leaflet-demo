...
module.exports = L.Control.extend({
    ...
    _initLayout: function _initLayout() {
        ...
        if (this.options.showValue) {
            if (window.matchMedia("screen and (-webkit-min-device-pixel-ratio:0)").matches && this.options.orientation == 'vertical') {
                this.slider.style.width = this.options.size.replace('px', '') - 36 + 'px';this._sliderContainer.style.height = this.options.size.replace('px', '') - 36 + 'px';
            ...
            if (window.matchMedia("screen and (-webkit-min-device-pixel-ratio:0)").matches && this.options.orientation == 'vertical') {
                this.slider.style.width = this.options.size.replace('px', '') - 10 + 'px';this._sliderContainer.style.height = this.options.size.replace('px', '') - 10 + 'px';
            } else if (this.options.orientation == 'vertical') {
        ...
        if (this.options.collapsed) {
            if (!L.Browser.android) {
                L.DomEvent.on(this._container, 'mouseenter', this._expand, this).on(this._container, 'mouseleave', this._collapse, this);
            }

            if (L.Browser.touch) {
                L.DomEvent.on(this._sliderLink, 'click', L.DomEvent.stop).on(this._sliderLink, 'click', this._expand, this);
            } else {
    ...
    _decrement: function _decrement() {
        console.log(this.slider.value - this.slider.step + " " + this.slider.value + this.slider.step);
        this.slider.value = this.slider.value * 1 - this.slider.step * 1;
        this._updateValue();
    }
